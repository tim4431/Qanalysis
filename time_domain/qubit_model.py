from Qanalysis.fitting.helper_tools import ScientificNumber, get_envelope
...
class Rabi(TimeSweep):
    ...
    def plot_result(self, fit_n_pts: int = 1000) -> plt.Figure:
        ...
        plt.title(f"$T_R$ = {TRabi_unitful}, $\Omega_R/2\pi$ = {OmegaRabi_2pi_unitful}")
        fig.tight_layout()
        plt.show()
        return fig
...
class PopulationDecay(TimeSweep):
    ...
    def fit_func(self, t: np.ndarray, T1: float, a: float, b: float) -> np.ndarray:
        """
    ...
    def _guess_init_params(self) -> None:
        ...
        T10 = (self.time[0] - self.time[mid_idx]) / np.log(
            1 - (self.signal[0] - self.signal[mid_idx]) / a0
        )
...
class Ramsey(TimeSweep):
    ...
    def fit_func(self, t: np.ndarray, *args) -> np.ndarray:
        ...
        if not self.fit_gaussian:
            ...
            return (
                a
                * np.exp(-t / Texp - (t / Tgauss) ** 2)
                * np.cos(2 * np.pi * df * (t - t0))
                + b
            )
    ...
    def _guess_init_params(self) -> None:
        ...
        if (
            env[-1] < env[0]
        ):  # sanity check: make sure the envelope is decreasing over time
            # try:
    ...
    def plot_result(self, fit_n_pts: int = 1000) -> plt.Figure:
        ...
        if not self.fit_gaussian:
            T2Ramsey_unitful = ScientificNumber(
                self.T2Ramsey, error=self.T2Ramsey_sigma_err, base_unit=self.time_unit
            )
...
class HahnEcho(TimeSweep):
    ...
    def _guess_init_params(self) -> None:
        ...
        mid_idx = np.argmin(np.abs(self.signal - (a0 / 2 + b0)))
        T20 = (self.time[0] - self.time[mid_idx]) / np.log(
            1 - (self.signal[0] - self.signal[mid_idx]) / a0
        )
...
class PowerRabi(AmpSweep):
    ...
    def fit_func(
        self, amp: np.ndarray, amp_pi: float, a: float, b: float
    ) -> np.ndarray:
        """
