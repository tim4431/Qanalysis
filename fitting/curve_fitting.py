...
from Qanalysis.fitting.helper_tools import ScientificNumber
...
class CurveFitAnalysis(DataAnalysis):
    ...
    def analyze(self, p0: np.ndarray = None, plot: bool = True, **kwargs) -> None:
        """
...
class CurveFitAnalysis1D(CurveFitAnalysis):
    ...
    def _plot_base(self, fit_n_pts: int = 1000) -> plt.Figure:
        ...
        x_unitful = ScientificNumber(
            np.max(np.abs(self._x["value"])), base_unit=self._x["unit"]
        )
        x_scaler = x_unitful.scaler
        x_scaled_unit = x_unitful.unit

        y_unitful = ScientificNumber(
            np.max(np.abs(self._y["value"])), base_unit=self._y["unit"]
        )
        ...
        # plot data
        plt.plot(
            self._x["value"] / x_scaler,
            self._y["value"] / y_scaler,
            ".",
            label="Data",
            color="black",
        )
        ...
        plt.plot(
            x_fit / x_scaler,
            self.fit_func(x_fit, *(self.p0)) / y_scaler,
        ...
        plt.plot(
            x_fit / x_scaler,
            self.fit_func(x_fit, *(self.popt)) / y_scaler,
    ...
    def _curve_fit(self, **kwargs) -> tuple[np.ndarray, np.ndarray]:
        if self.lb is not None and self.ub is not None:
            ...
            popt, pcov = curve_fit(
                self.fit_func, self._x["value"], self._y["value"], p0=self.p0, **kwargs
            )
        return popt, pcov
